...
def filter(pcd, max_radius, z_min, z_max):
    ...
    """
    points = np.array(pcd.points)  # 改用 np.array() 以确保返回可写副本
...
def clamp_euler(T, max_roll_deg=30, max_pitch_deg=30, max_yaw_deg=30):
    """
...
class PointCloudRegistrationNode(object):
    def __init__(self):
        ...
        self.angle_range = np.deg2rad(np.arange(45, 96, 15))  # 45° 到 90°（步长15°）
    ...
    def pointcloud2_to_o3d(self, pc2_msg):
        ...
        """
        pc = np.array(ros_numpy.point_cloud2.pointcloud2_to_array(pc2_msg))
    ...
    def handle_registration(self, req):
        ...
        best_T, best_fitness = self.try_registration()
    ...
    def try_registration(self):
        ...
        for center_angle in self.angle_range:
            ...
            T_limited = clamp_euler(T_global, max_roll_deg=10, max_pitch_deg=10, max_yaw_deg=130)
            T_global = T_limited

            candidate_aligned_global = copy.deepcopy(candidate_down)
            candidate_aligned_global.transform(T_global)

            source_np = np.array(candidate_aligned_global.points)
            target_np = np.array(pcd_sector_down.points)

            # 使用自定义 constrained_icp 在 XY 平面内精细对齐
            T_icp = constrained_icp(source_np, target_np, max_iter=50, tolerance=1e-6)
            T_total = T_icp @ T_global

            # 计算均值最近邻距离作为匹配误差
            source_hom = np.hstack((np.array(candidate_down.points),
                                    np.ones((np.array(candidate_down.points).shape[0], 1))))
            ...
            rospy.loginfo("Candidate sector center angle {:.1f}° average error: {:.4f}".format(np.rad2deg(center_angle), fitness))
